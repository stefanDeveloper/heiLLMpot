...
inline ssize_t write_headers(Stream &strm, const Headers &headers) {
  ...
  // Custom rank-based honeypot evasion patch: realistically order common headers
  const char* preferred_order[] = {
      "Server", "Date", "Content-Type", "Content-Length", 
      "Connection", "Keep-Alive", "Cache-Control", 
      "Pragma", "X-Powered-By", "X-Content-Type-Options", 
      "X-Frame-Options", "Strict-Transport-Security"
  };

  for (const auto &p : preferred_order) {
    for (const auto &x : headers) {
      if (iequals(x.first, p)) {
